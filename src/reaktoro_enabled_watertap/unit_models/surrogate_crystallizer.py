...
from reaktoro_enabled_watertap.costing.surrogate_crystallizer import (
...
see property package for documentation.}""",
    ...
    CONFIG.declare(
        "include_soft_mass_balance_constraints",
        ConfigValue(
            default=True,
            domain=bool,
            description="Use soft_min function for mass removal calculations (prevents 100% removal of given ion)",
            doc="""Use soft_min function for mass removal calculations (prevents 100% removal of given ion).""",
        ),
    )
    ...
    def build(self):
        ...
        for m in self.solids_list:
            for p in self.config.property_package.component_list:
                dict1[m, p] = float(
        ...
        def eq_solid_amount(b, j):
            ...
            for _q in b.solids_list:
                if b.mwc[_q, j] > 0:
                    ...
                    total_solid = []
                    for _j in b.config.property_package.ion_set:
                        if b.mwc[_q, _j] > 0:
                            total_solid.append(
                                b.mwc[_q, _j] * b.properties_in[0].params.mw_comp[_j]
                            )
                    ...
                    total.append(
                        ...
                        * b.properties_in[0].params.mw_comp[j]
                        / sum(total_solid)
                        * b.flow_mass_sol_comp_apparent[_q]
                    )
            if self.config.include_soft_mass_balance_constraints:
                flow_in = b.properties_in[0].flow_mass_phase_comp["Liq", j]
                return b.flow_mass_sol_comp_true[j] == idaesMath.smooth_min(
                    sum(total), flow_in * (1 - 1e-8), 1e-8
                )
            else:
                return b.flow_mass_sol_comp_true[j] == sum(total)
        ...
        def eq_heat_duty(b):
            return pyunits.convert(b.heat_duty, to_units=pyunits.W) == sum(
                b.properties_out_vapor[0].enth_flow_phase[p]
                for p in self.config.vapor_property_package.phase_list
            )
        ...
        # )
        # def eq_energy_balance_constraint(b):
        #     return b.heat_required + b.properties_in[
        #         0
    ...
    def initialize_build(
        ...
        if (
            self.config.property_package.config.material_flow_basis
            == MaterialFlowBasis.molar
        ):
            self.properties_in[0].flow_mass_phase_comp[...]
            self.properties_out_liq[0].flow_mass_phase_comp[...]
        ...
        for p in self.config.vapor_property_package.phase_list:
            if p == "Vap":
                if (
                    ...
                    value = (
                        self.properties_in[0].flow_mass_phase_comp["Liq", "H2O"].value
                    )
                else:
                    value = state_args["flow_mass_phase_comp"]["Liq", "H2O"]
                state_args_vapor["flow_mass_phase_comp"][p, "H2O"] = value
            else:
    ...
    def calculate_scaling_factors(self):
        ...
        for j in self.flow_mass_sol_comp_true:
            ...
            iscale.constraint_scaling_transform(
                self.eq_solid_amount[j],
                sf,
            )
        # for solid in self.flow_mass_sol_comp_apparent:
        #     sf = iscale.get_scaling_factor(self.flow_mass_sol_comp_apparent[solid])
        #     if sf is None:
        #         sf_total = []
        #         for ion, stoich in self.config.solids_ions_dict[solid].items():
        #             ion_sf = iscale.get_scaling_factor(
        #                 self.flow_mass_sol_comp_true[ion]
        #             )
        #             sf = ion_sf / self.properties_in[0].params.mw_comp[ion] / stoich
        #             sf_total.append(sf)
        #         iscale.set_scaling_factor(
        #             self.flow_mass_sol_comp_apparent[solid], max(sf_total)
        #         )
        #     sf_c = iscale.get_constraint_transform_applied_scaling_factor(
        #         self.eq_solid_formation[solid]
        #     )
        #     if sf_c is None:
        #         sf = iscale.get_scaling_factor(self.flow_mass_sol_comp_apparent[solid])
        #         iscale.constraint_scaling_transform(self.eq_solid_formation[solid], sf)
        if iscale.get_scaling_factor(self.flow_mass_sol_total) is None:
            iscale.set_scaling_factor(self.flow_mass_sol_total, max(solid_sf))
        if (
            iscale.get_constraint_transform_applied_scaling_factor(
        ...
        for j, c in self.eq_energy_balance_constraint.items():
            ...
            iscale.constraint_scaling_transform(c, sf)
            iscale.set_scaling_factor(self.heat_duty, sf)
            iscale.constraint_scaling_transform(self.eq_heat_duty, sf)
